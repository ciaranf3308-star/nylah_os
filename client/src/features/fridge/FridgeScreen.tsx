...
function FridgePage({
  ...
  // helpers for remaining sections (chores deck, sticky, empty)
  // kept identical to original for zero logic change
  ...
  // For pantry low & hasToday recompute (same logic as Upcoming to preserve condition)
  ...
  return (
    <div className="w-full space-y-6">
        ...
        <div className="relative mt-5">
          ...
          <h1 className="nylah-display-hero text-[46px] -mt-1" style={{fontFamily:'var(--font-display)', color:'var(--text)'}}>
            ...
            <span className="ml-2 inline-flex items-baseline gap-2 align-baseline">
              ...
              <span className="nylah-script-hero text-[34px] font-script-hero" style={{fontFamily:"var(--font-script)", color:'var(--accent-warm)'}}>{PERSONS[partner]?.name||partner}</span>
            </span>
          </h1>
      ...
      {/* NEEDS YOU — extracted */}
      <NeedsYou currentUser={currentUser} calendar={activeCalendar as any} chores={activeChores as any} nowMs={nowMs} setTab={setTab} />

      {/* TODAY + UPCOMING — extracted */}
      <Upcoming currentUser={currentUser} calendar={activeCalendar as any} chores={activeChores as any} shopping={activeShopping as any} nowMs={nowMs} todayDateStr={todayDateStr} setTab={setTab} />

      {/* AWAITING YOU (tentative) — extracted, preserves original order: after Upcoming */}
      <AwaitingYou currentUser={currentUser} calendar={activeCalendar as any} nowMs={nowMs} setTab={setTab} />

      {/* PINNED & COUNTDOWNS — extracted */}
      <Countdowns calendar={activeCalendar as any} nowMs={nowMs} todayDateStr={todayDateStr} setTab={setTab} />
