      ...
      <div className="w-full max-w-[360px] rounded-[28px] border bg-[var(--card-bg)] shadow-[0_18px_50px_rgba(0,0,0,0.10)] px-6 py-7 flex flex-col items-center" style={{ borderColor:"var(--border)" }}>
          ...
          <div className="w-full -m-7 flex min-h-dvh flex-col bg-[#FEF7F0] overflow-hidden">
            <div className="flex flex-col items-center pt-[48px] px-6">
              ...
              <div className="mt-1.5 flex items-center gap-2 text-[10.5px] font-semibold tracking-[0.18em] uppercase"><span className="text-[#8AA99B]">HOME</span><span className="h-[2.5px] w-[2.5px] rounded-full bg-[#8B7357]/60"/><span className="text-[#C97A48]">PEOPLE</span><span className="h-[2.5px] w-[2.5px] rounded-full bg-[#8B7357]/60"/><span className="text-[#151210]">TOGETHER</span></div>
            ...
            <div className="w-full px-[18px] pb-[18px] mt-auto bg-[#FEF7F0]">
              <div className="mx-auto w-full max-w-[384px] rounded-[26px] bg-[#101214] px-[18px] pt-[14px] pb-[18px] border border-white/[0.04]">
                ...
                <div className="mt-4 flex items-center gap-3 px-1"><div className="flex items-center gap-2 flex-1"><span className="grid h-[22px] w-[22px] place-items-center rounded-full bg-white/[0.06] border border-white/[0.06]"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#A8C5B5" strokeWidth="1.7"><rect x="4" y="10" width="16" height="10" rx="2"/><path d="M8 10V7a4 4 0 0 1 8 0v3"/></svg></span><span className="text-[11px] leading-[1.32] text-[#A9BFB3]">Your data stays in your own household.<br/>Each code is private.</span></div><div className="h-[28px] w-[1px] bg-white/[0.08]"/><span className="grid h-[24px] w-[24px] place-items-center"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#A8C5B5" strokeWidth="1.6"><path d="M12 3l7 3v5c0 5-3.5 8-7 10-3.5-2-7-5-7-10V6l7-3z"/><path d="M9.5 12.5l2 2 4-4" strokeLinecap="round" strokeLinejoin="round"/></svg></span></div>
              </div>
            </div>
          </div>
        )}        {step==="create_names" && (
...
function CalendarPageV2({
  ...
  async function removeEvent(idOrEv:any) {
    ...
    if (!target) target = (events as any[]).find((e:any)=> e.id===id) || null;
    if (!target) {
      try {
    ...
    if (isGeneratedOccurrence) {
      ...
      const override = { id: ovId, templateId: tplId, occurrenceId: occId, isOverride: true, deletedAt: new Date().toISOString(), updatedAt: new Date().toISOString(), updatedBy: currentUser, title: (target as any).title, start: (target as any).start, dueAt: (target as any).dueAt } as any;
      setEvents((prev:any)=> {
        const without = (prev as any[]).filter((e:any)=> !(e.templateId===tplId && e.occurrenceId===occId));
        return [...without, override];
      });
      try { const { upsertCalendarOverride } = await import("./lib/normalized"); await upsertCalendarOverride({ id: ovId, seriesId: tplId, series_id: tplId, occurrenceDate: occId, occurrence_date: occId, deleted: true, title: (target as any).title, data: override } as any); } catch {}
      return;
    }
    if (isTemplate) {
      setEvents((prev:any)=> prev.map((ev:any)=> ev.id===id || ev.templateId===id ? { ...ev, deletedAt:new Date().toISOString(), updatedAt:new Date().toISOString(), updatedBy: currentUser } : ev));
      try {
        const { getSupabase, getEffectiveRowId } = await import("./lib/supabase");
        const sb = getSupabase(); const hid = getEffectiveRowId() || (target as any).household_id;
        if (sb && hid) { try{ await (sb as any).from('calendar_series').delete().eq('id', id).eq('household_id', hid);}catch{} try{ await (sb as any).from('calendar_occurrence_overrides').delete().eq('series_id', id).eq('household_id', hid);}catch{} try{ await (sb as any).from('calendar_events').delete().eq('id', id).eq('household_id', hid);}catch{} }
      } catch {}
      return;
    }
    ...
    try {
      const { getSupabase, getEffectiveRowId } = await import("./lib/supabase");
      const sb = getSupabase(); const hid = getEffectiveRowId() || (target as any)?.household_id || localStorage.getItem('couple_v1_household_id');
      if (sb && hid) { try{ await (sb as any).from('calendar_events').delete().eq('id', id).eq('household_id', hid);}catch{} try{ await (sb as any).from('calendar_events').update({ deleted_at: new Date().toISOString() } as any).eq('id', id).eq('household_id', hid);}catch{} }
    } catch {}
  }
