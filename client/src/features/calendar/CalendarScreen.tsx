...
function PinnedCalendarCard({ ev, nowMs, onClear, onTap, tz }: { ev:any, nowMs:number, onClear:()=>void, onTap:()=>void, tz:string }){
  ...
  return (
    <button onClick={onTap} className="group w-full text-left rounded-[24px] border bg-[#FFFEFB] px-5 pt-4 pb-5 min-h-[172px] relative overflow-hidden shadow-[0_12px_30px_rgba(60,40,20,0.10),0_1px_0_rgba(255,255,255,0.9)_inset] hover:shadow-[0_18px_42px_rgba(60,40,20,0.15)] hover:-translate-y-[0.5px] transition-all" style={{borderColor:"#EDE2D6"}}>
      ...
      <div className="pointer-events-none absolute right-[-14px] bottom-[-18px] rotate-[-6deg] group-hover:rotate-[-3deg] group-hover:scale-[1.02] transition-transform duration-500">
        <div className="relative" style={{ width:188, height:188 }}>
          <div className="absolute inset-[16px] rounded-[30px] rotate-[7deg]" style={{ background:`radial-gradient(115% 90% at 32% 28%, white 0%, ${pal.bg} 48%, ${pal.chipBg||pal.bg} 82%)`, opacity:0.84 }} />
          <div className="relative w-full h-full grid place-items-center">
            <EventIcon kind={kind} title={title} size={172} variant="watermark" theme="light"/>
          </div>
        </div>
      </div>
    </button>
