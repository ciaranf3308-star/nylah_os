        ...
        {detailChore && (
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
                ...
                const nowISO=new Date().toISOString(); const doneUpd:any={...detailChore, status:"done", completedBy: currentUser, completedAt:nowISO, updatedAt:nowISO, updatedBy:currentUser}; setChores((p:any)=> p.map((x:any)=> x.id===detailChore.id ? doneUpd : x)); try{ hardPersistChore(doneUpd,'update'); }catch{} setDetailChore(null); const pts=effectivePoints(detailChore,false); triggerPointsPop(detailChore.id, pts); confettiByPoints(pts); try{ import('../../lib/push').then(m=> (m as any).notifyOther(currentUser as any, {title: `${(currentUser==='aisling'?'Aisling':'Ciarán')} did ${detailChore.title}`, body: `+${pts} pts • ${monthKey}`, url: './?standalone'})) }catch{} }} className="flex-1 h-[52px] rounded-[16px] bg-[#0A0A0A] text-white text-[13px] font-semibold active:scale-[0.96]" style={{minHeight:52, transition:"transform 180ms cubic-bezier(0.34,1.56,0.64,1)"}}>Mark done • +{effectivePoints(detailChore,false)}</button>
              <button onClick={()=> { const nowISO=new Date().toISOString(); const upd:any={...detailChore, status:"deck", swipes:{aisling:null,ciaran:null}, updatedAt:nowISO, updatedBy:currentUser}; setChores((p:any)=> p.map((x:any)=> x.id===detailChore.id ? upd : x)); try{ hardPersistChore(upd,'update'); }catch{} setDetailChore(null); triggerPointsPop(detailChore.id, 20); confettiByPoints(20); }} className="flex-1 h-[44px] rounded-[16px] border bg-[var(--card-bg)] text-[13px] active:scale-[0.96]" style={{borderColor:"var(--border)", minHeight:44}}>Reshuffle</button>
            </div>
            <div className="flex gap-2">
              ...
              <button onClick={()=>{ const nowISO=new Date().toISOString(); const half1={...detailChore, id: uid("chk"), title: detailChore.title+" • A", pain: Math.ceil(detailChore.pain/2), basePoints: Math.ceil(detailChore.basePoints/2), updatedAt:nowISO, createdAt:nowISO, swipes:{aisling:null,ciaran:null}, status:"deck", assignedTo:null, multiplier:1, timeWindowHours:24, type: detailChore.type, frequency: detailChore.frequency, frequencyDetail: detailChore.frequencyDetail, icon: (detailChore as any).icon}; const half2={...detailChore, id: uid("chk"), title: detailChore.title+" • B", pain: Math.floor(detailChore.pain/2)||1, basePoints: Math.floor(detailChore.basePoints/2)||5, updatedAt:nowISO, createdAt:nowISO, swipes:{aisling:null,ciaran:null}, status:"deck", assignedTo:null, multiplier:1, timeWindowHours:24, type: detailChore.type, frequency: detailChore.frequency, frequencyDetail: detailChore.frequencyDetail, icon: (detailChore as any).icon}; setChores((p:any)=> [half1, half2, ...p.filter((x:any)=> x.id!==detailChore.id)]); try{ hardPersistChore(half1,'create'); hardPersistChore(half2,'create'); hardPersistChore({id: detailChore.id, deletedAt: nowISO, updatedAt: nowISO} as any,'delete'); }catch{} setDetailChore(null); setToast("Split into two"); setTimeout(()=>setToast(null),2000); }} className="flex-1 h-[44px] rounded-full border bg-[var(--card-bg)] text-[11px]" style={{borderColor:"var(--border)", minHeight:44}}>Split • Two</button>
            </div>
