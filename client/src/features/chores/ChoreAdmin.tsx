    ...
    <div className="space-y-4">
      <div className="rounded-[28px] border bg-[var(--card-bg)] p-4 space-y-3" style={{borderColor:"var(--border)", background:"linear-gradient(180deg,var(--wash-mid) 0%,var(--card-bg) 36%,var(--card-bg) 100%)", boxShadow:"0 16px 40px rgba(0,0,0,0.12)"}}>
        ...
        <div className="mt-2 grid grid-cols-2 gap-2">
          <button onClick={()=> { const nowISO=new Date().toISOString(); const all=active.map((c:any)=> ({...c, swipes:{aisling:null, ciaran:null}, status:"deck", updatedAt:nowISO, updatedBy:currentUser})); setChores((p:any)=> { const m=new Map(all.map((a:any)=>[a.id,a])); return (p as any[]).map((x:any)=> m.has(x.id) ? m.get(x.id) : x); }); try{ all.forEach((a:any)=>{ try{ hardPersistChoreAdmin(a,'update'); }catch{} }); }catch{} setToast("Reshuffled");  setTimeout(()=>setToast(null),2000); }} className="h-[44px] rounded-full border bg-[var(--card-bg)] text-[11px] font-semibold active:scale-[0.96]" style={{borderColor:"var(--border)", minHeight:44}}>Reshuffle all</button>
          <button onClick={()=> { const kept = chores.filter((c:any)=> c.status!=="done" || (c.completedAt && toLocalKeyDublin(c.completedAt, HOUSEHOLD_TZ).startsWith(monthKey))); const dropped = chores.filter((c:any)=> !kept.includes(c as any)); setChores(kept); try{ dropped.forEach((d:any)=>{ try{ hardDeleteChore(String(d.id)); }catch{} }); }catch{} setToast(`Archived old • kept ${monthKey}`); setTimeout(()=>setToast(null),2000); }} className="h-[44px] rounded-full bg-[#0A0A0A] text-white text-[11px] font-semibold active:scale-[0.96]" style={{minHeight:44}}>Archive old • {monthKey}</button>
        </div>
      </div>
